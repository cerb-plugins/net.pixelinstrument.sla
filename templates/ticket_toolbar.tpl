{if $ticket_sla_info.sla_days > 0}
	...
	<span id="ticket_sla_first_response">
		<strong>First Response:</strong>
			{if $ticket_sla_info.sla_type == "b"}
				<span style="{if $ticket_sla_info.response_business_days == -1 && $smarty.now <= $ticket_sla_info.sla_days}color:#CA0;font-weight:bold;{else if $ticket_sla_info.response_business_days == -1 || $ticket_sla_info.response_business_days > $ticket_sla_info.sla_days}color:#C00;font-weight:bold;{/if}">
			...
			{else}
				<span style="{if $ticket_sla_info.response_days == -1 && $smarty.now <= $ticket_sla_info.sla_days}color:#CA0;font-weight:bold;{else if $ticket_sla_info.response_days == -1 || $ticket_sla_info.response_days > $ticket_sla_info.sla_days}color:#C00;font-weight:bold;{/if}">
	...
	<span id="ticket_sla_last_response">	
		<strong>Last Response:</strong>
			<span style="{if $ticket_sla_info.last_response_time == -1}color:#CA0;font-weight:bold;{/if}">
